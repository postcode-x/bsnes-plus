...
namespace nall {
	...
	struct SNESCPU {
		...
		static string disassembleRaw(unsigned pc, bool accum, bool index, uint8_t opcode, uint8_t pl, uint8_t ph, uint8_t pb);
	};
	...
	inline string SNESCPU::disassembleRaw(unsigned pc, bool accum, bool index, uint8_t opcode, uint8_t pl, uint8_t ph, uint8_t pb) {
		string name = cpuOpcodeInfo[opcode].name;
		unsigned mode = cpuOpcodeInfo[opcode].mode;

		if (mode == Implied) return { hex<2>(opcode) };
		if (mode == Constant) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == AccumConstant) return { hex<2>(opcode), " ", accum ? "" : hex<2>(ph), hex<2>(pl) };
		if (mode == IndexConstant) return { hex<2>(opcode), " ", index ? "" : hex<2>(ph), hex<2>(pl) };
		if (mode == Direct) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == DirectX) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == DirectY) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == IDirect) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == IDirectX) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == IDirectY) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == ILDirect) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == ILDirectY) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == Address) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == AddressX) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == AddressY) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == IAddressX) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == ILAddress) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == PAddress) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == PIAddress) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == Long) return { hex<2>(opcode), " ", hex<2>(pb), hex<2>(ph), hex<2>(pl) };
		if (mode == LongX) return { hex<2>(opcode), " ", hex<2>(pb), hex<2>(ph), hex<2>(pl) };
		if (mode == Stack) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == IStackY) return { hex<2>(opcode), " ", hex<2>(pl) };
		if (mode == BlockMove) return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		if (mode == RelativeShort) {
			unsigned addr = (uint16_t)(pc + 2) + (int8_t)(pl << 0);
			return { hex<2>(opcode), " ", hex<2>(pl) };
		}
		if (mode == RelativeLong) {
			unsigned addr = (uint16_t)(pc + 3) + (int16_t)((ph << 8) + (pl << 0));
			return { hex<2>(opcode), " ", hex<2>(ph), hex<2>(pl) };
		}
